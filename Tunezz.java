...
//This program uses the concept of linked lists to execute the operations. A linked list is a dynamic data structure that has nodes
//where each node has a data value and a reference to the next node.
...
public class Tunezz {
	...
	private static Playlist findPlaylist(String name, ArrayList<Playlist> playlists) {
	...
	public static void main(String[] args) {
		...
		//Print out the menu
		System.out.println("Welcome to Tunezzzz!");
		System.out.println("===================");
		printMenu();
		...
		do {
			try {
				...
				case "add": // Add to current playlist
					song = parseSong(Arrays.copyOfRange(input, 1, input.length));
				...
				case "new":
					...
					break;
				case "count":
					System.out.printf("The current playlist %s has %d songs.\n",currentPlaylist.getName(), currentPlaylist.size());
					break;
				case "position":
					song = parseSong(Arrays.copyOfRange(input, 1, input.length));
					...
					if(result == -1) {
						...
						System.out.printf("%s song is located at position %d in %s\n", song, result+1, currentPlaylist.getName());
					}
					break;
				case "remove":
					song = parseSong(Arrays.copyOfRange(input, 1, input.length));
